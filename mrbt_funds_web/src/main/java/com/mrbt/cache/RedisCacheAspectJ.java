...
public class RedisCacheAspectJ {
	...
	public Object methodCacheGetAndHold(ProceedingJoinPoint joinPoint)
			throws Throwable {
		...
		EvaluationContext ec = getContext(methodSignature.getParameterNames(),
				arguments);
		RedisCache redisCache = methodSignature.getMethod().getAnnotation(
				RedisCache.class);
		// 获取到key的值
		String key = StringUtils.isBlank(redisCache.key()) ? null : parser
				.parseExpression(redisCache.key()).getValue(ec, String.class);
		// 保存对象的annotation
		...
		if (put != null && StringUtils.isNotBlank(put.saveCondition())) {
			saveFlag = parser.parseExpression(put.saveCondition()).getValue(ec,
					Boolean.class);
		}
	...
	public Object methodCacheHold(ProceedingJoinPoint joinPoint)
		...
		EvaluationContext ec = getContext(methodSignature.getParameterNames(),
				arguments);
		RedisPut put = methodSignature.getMethod()
				.getAnnotation(RedisPut.class);
		// 获取到key的值
		String key = StringUtils.isBlank(put.key()) ? null : parser
				.parseExpression(put.key()).getValue(ec, String.class);
		if (put != null && StringUtils.isNotBlank(key)
				&& StringUtils.isNotBlank(put.saveCondition())) {
			saveFlag = parser.parseExpression(put.saveCondition()).getValue(ec,
					Boolean.class);
		}
	...
	public Object methodCacheEvict(ProceedingJoinPoint joinPoint)
			throws Throwable {
		MethodSignature methodSignature = (MethodSignature) joinPoint
				.getSignature();
		Object[] arguments = joinPoint.getArgs();
		RedisCacheEvict evict = methodSignature.getMethod().getAnnotation(
				RedisCacheEvict.class);
		EvaluationContext ec = getContext(methodSignature.getParameterNames(),
				arguments);
		String key = parser.parseExpression(evict.key()).getValue(ec,
				String.class);
