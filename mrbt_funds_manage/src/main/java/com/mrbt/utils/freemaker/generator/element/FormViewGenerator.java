...
public class FormViewGenerator extends BaseGenerator {

	public FormViewGenerator(XmlBean bean) {
		super(bean);
	...
	public boolean generatorHtml(String ftlPath) {
		try {
			...
			contextMap.put("viewList", generatorList());
			contextMap.put("comboMap", bean.comboMaps);
			contextMap.put("hight", getHeight(generatorList().size()));
			generator.geneHtmlFile("view.ftl", ftlFile.getPath(), contextMap,
					getPathWithEnd(bean.path.jsp), "view.jsp");
			return true;
		} catch (Exception ex) {
			ex.printStackTrace();
		}
